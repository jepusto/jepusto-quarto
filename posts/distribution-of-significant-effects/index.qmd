...
V_NA_approx = m * psi * (1 - psi) + m * (m - 1) * V_pi_approx
...
\begin{aligned}
\psi &= ${psi} \\
\mathbb{E}\left(N_A\right) &= ${E_NA} \\
\mathbb{V}\left(N_A\right) &= ${V_NA} \\
V_{approx} &= ${V_NA_approx}
\end{aligned}
`
```
:::
...
$$
\begin{aligned}
\text{Var}(N_A) \approx V_{approx} = m \times \psi \left(1 - \psi\right) + m (m - 1) \times \left[\phi\left(\frac{\mu - q_{\alpha} \sigma}{\sqrt{\omega^2 + (1 - \rho)\sigma^2}}\right)\right]^2 \times \frac{\tau^2 + \rho \sigma^2}{\omega^2 + (1 - \rho)\sigma^2}.
\end{aligned}
$$

...
$$
\begin{aligned}
KL_N\left(SN\left(\xi,\omega,\alpha\right), N\left(0,1\right)\right) &= \mathbb{E}_{SN}\left[\log f_{SN}(X) - \log \phi(X)\right] \\
&= \log(2) + \frac{1}{2} \log(1 - a^2) \\
& \qquad  + \mathbb{E}_{SN}\left[ \frac{1}{2} X^2 - \frac{1}{2}\left(X \sqrt{1 - a^2} + a\right)^2 \right. \\
& \qquad \qquad \qquad  + \left. \log \Phi\left(\alpha \left(X \sqrt{1 - a^2} + a\right)\right) \right] \\
&= \log(2) + \frac{1}{2} \log\left(1 - a^2\right) \\
& \qquad \qquad + \mathbb{E}_{SN}\left[ \log \Phi\left(\alpha \left(X \sqrt{1 - a^2} + a\right)\right) \right].
\end{aligned}
$$
...
Plot.plot({
  ...
  marks: [
    ...
    Plot.areaY(dat3, {x: "x", y: "q", fill: "blue", fillOpacity: 0.5}),
    Plot.lineY(dat3, {x: "x", y: "q", stroke: "blue"}),
    Plot.areaY(dat3, {x: "x", y: "r", fill: "orange", fillOpacity: 0.5}),
    Plot.lineY(dat3, {x: "x", y: "r", stroke: "orange"})
  ]
})
```

...
$$
which is the overall probability of observing an effect size estimate in a study with standard error $\sigma_i^*$.^[@hedges2017plausibility denotes this quantity $g_i(T_i)$.]
...
Hedges approaches this by assuming that an effect size estimate with $p$-value equal to the smallest $p$-value will be reported with certainty, so that the proportionality constant is $w_{min} = (p_{min})^{\lambda_1 - 1} (1 - p_{min})^{\lambda_2 - 1}$.
...
Another, related limitation of the beta density model is that it puts too much emphasis on a few individual observations that have low probability of selection. 
...
$$
I report these relative weights in the last column of @tbl-Hedges-Table1. 
...
fit_fun <- function(alpha1, alpha2 = 1 - alpha1) {
  ...
  summary_measures <- data.frame(
    param = c("k_gen","omega_8","omega_23", "log-likelihood"),
    Est = c(sum(Table1$Ni), Table1$omega[8], Table1$omega[23], as.numeric(logLik(beta_fit)))
  )
...
trunc_points <- expand_grid(
  alpha1 = c(1e-5, 5e-5, 1e-4, 5e-4, 1e-3, 5e-3, 1e-2, 0.025),
  alpha2 = c(NA, 0.5)
) %>%
  mutate(
...
beta_sens <- 
  trunc_points %>%
  mutate(
    res = future_pmap(., fit_fun, .progress = TRUE)
  ) %>%
  unnest(res)
...
beta_sens_clean <- 
  beta_sens %>%
  filter(param %in% c("beta","tau2","log-likelihood", "k_gen", "omega_8", "omega_23")) %>%
  mutate(
    alpha2 = if_else(alpha2 == 0.5, "alpha[2] == 0.5", "alpha[2] == 1 - alpha[1]"),
    param = factor(
      param, 
      levels = c("log-likelihood", "beta", "tau2","k_gen", "omega_8", "omega_23"), 
      labels = c("log-likelihood","mu","tau^2","k[gen]","omega[8]","omega[23]")
    )
  )

...
dat <- sim_moments(reps = 100000, delta = 0.25, N = 12, p = 1/2)
...
```{r}
#| label: fig-true-SE-zoom
#| fig-width: 8
#| fig-height: 8
#| fig-cap: "True standard errors of transformed ($z(d)$ and $h(d)$) effect size estimates"

sim_d_moments |>
  mutate(SE = if_else(stat == "d", SE * sqrt(p * (1 - p)), SE)) |>
ggplot() + 
  aes(delta, SE, color = stat) + 
  expand_limits(y = 0) + 
  geom_line() + 

---
title: Notes on regression adjustment for selectively reported results
date: '2025-08-09'
categories:
- effect-size
- distribution-theory
...
Several of the most prominent methods for investigating selective reporting bias in meta-analyis take the form of simple linear regressions. 
...
By my read, the simulation evidence is mixed---they are better than regular random effects models but they are pretty crude adjustments for the bias created by selective reporting, which don't always work very well.
In this post, I want to look at how the methods work _analytically_. I'll derive expressions for their biases and consider how one might use these expressions to say something about the conditions under which they might be expected to perform reasonably well. 
...
The intuition behind this sort of regression adjustments is that selective reporting on the basis of statistical significance tends to produce larger biases for smaller primary studies, which have larger standard errors, leading to what looks like "small-study" effects. 
...
Denote the full vector of standard errors as $\boldsymbol\sigma = \left(\sigma_1,...,\sigma_K\right)'$ and noting that the predictors are all functions of the standard errors, we can then write the conditional expectation and conditional variance of $\hat\alpha$ as
$$
...
These different regression adjustments are a bit hard to pin down because they're not full generative models.
...
Consequently, we have to look beyond these specific methods to other models for the selective reporting process if we want to understand how well the regression adjustments work.
...
Here I'll look at a step-function model and the Copas model, mostly because these are a bit more tractable to analyze.
...
$$
with $c_{0i} = \infty$ and $c_{H+1,i} = -\infty$.
For a model with a single step at $\alpha_1$ and a single selection parameter $\lambda_1$,
$$
\psi(\mu,\tau,\lambda_1, \sigma_i) = \frac{(1 - \lambda_1) \phi(c_{1i})}{1 - (1 - \lambda_1)\Phi(c_{1i})}.
$$
Substituting into @eq-alpha-expectation, the bias of a regression adjustment under the step-funtion selection model is
$$

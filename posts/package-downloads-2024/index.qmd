...
downloads_past_twelve <-
  ...
  mutate(
    ...
    package_sort = fct_rev(fct_infreq(package, w = count))
  )
```
...
downloads_past_twelve_with_groups <-
  downloads_past_twelve %>%
  left_join(pkg_groups) %>%
  mutate(
    package = fct_rev(fct_infreq(package, w = count)),
    group = recode(group, .missing = "none"),
...
downloads_past_twelve_plot <- 
  downloads_past_twelve %>%
  select(package = package_sort, count) 

p <- 
  ggplot(downloads_past_twelve_plot) +
  aes(x = package, y = count) + 

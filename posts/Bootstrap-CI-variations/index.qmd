...
ggplot(boot_performance) +
  ...
  geom_point() + 
  ggrepel::geom_label_repel(data = boot_performance_top, aes(label = CI_type), nudge_x = -0.004) + 
...
```{r, fig.width = 7, fig.height = 4, out.width = "100%"}
boot_performance_top <- filter(boot_performance, bootstraps == 49)
...
toc()
```
There is some time savings, but perhaps not worth the extra hassle. 

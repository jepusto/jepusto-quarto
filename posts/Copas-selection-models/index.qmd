...
$$
\Pr\left(T_i = t| \sigma_i^*\right) = \frac{1}{\sqrt{\tau^2 + \left(\sigma_i^*\right)^2}} \phi\left(\frac{T_i^* - \mu}{\sqrt{\tau^2 + \left(\sigma_i^*\right)^2}}\right) \times \frac{\Phi\left(\frac{1}{\sqrt{1 - \tilde\rho_i^2}}\left[\alpha + \frac{\beta}{\sigma_i^*} + \tilde\rho_i \left(\frac{T_i^* - \mu}{\sqrt{\tau^2 + \left(\sigma_i^*\right)^2}}\right)\right]\right)} {\Phi\left(\alpha + \frac{\beta}{\sigma_i^*}\right)}
$$ {#eq-observed-density}
...
viewof alpha = Inputs.range(
  [-5, 5],
  {value: 0.0, step: 0.1, label: "alpha selection parameter"}
)

viewof beta = Inputs.range(
  [0, 2],
  {value: 1, step: 0.01, label: "beta selection parameter"}
)

...
They frame the method as a generalization of an earlier proposal of theirs, called p-uniform, which was limited in that it applies only to meta-analytic data where all effect size estimates are statistically significant at the conventional $\alpha = .05$ level.
...
In the new paper, the authors contrast p-uniform* with the well-known three-parameter selection model (3PSM), which is a special case of the [step-function selection model](/posts/step-function-selection-models/) introduced by @hedges1992modeling.
...
Finally, I will use $\Phi()$ to denote the cumulative function and $\phi()$ to denote the density function of the standard normal distribution.

# The three-parameter selection model

To see how p-uniform\* relates to the 3PSM, I will explain the latter model.
...
$$
d_T(t, a_i | \sigma_i) = \frac{ \lambda^{1 - a_i}}{1 - (1 - \lambda)B_i} \times \frac{1}{\sqrt{\tau^2 + \sigma_i^2}}\phi\left(\frac{t - \mu}{\sqrt{\tau^2 + \sigma_i^2}}\right),
$$ {#eq-observed-effect-density}
where $a_i = I\left(\frac{t}{\sigma_i} > \Phi^{-1}(1 - \alpha)\right)$ and $B_i = \Phi\left(\frac{\sigma_i \Phi^{-1}(1 - \alpha) - \mu}{\sqrt{\tau^2 + \sigma_i^2}}\right)$.
...
& \qquad \qquad \qquad + k_N \log \lambda - \sum_{i=1}^k \log\left(1 - (1 - \lambda) B_i\right)
\end{aligned}
$$ {#eq-3PSM-log-lik}
where $k_N = \sum_{i=1}^k (1 - A_i)$ is the number of non-affirmative effect size estimates observed.
The maximum likelihood estimators of the model parameters are the values that maximize $\mathcal{l}(\mu, \tau, \lambda)$. 
...
With profiling, we think of the maximum likelihood estimator of $\lambda$ as a function of $\mu$ and $\lambda$, as
$$
...
and the maximum likelihood estimators of $\mu$ and $\tau$ are the values that maximize $l_P(\mu, \tau)$. 
...
$$
k_A = \sum_{i=1}^k \frac{1 - B_i}{1 - (1 - \lambda) B_i}.
...
The method is an estimator of $\mu$ and $\tau$, but based on a different objective function that is based on truncated normal distributions [Equation 3 of @VanAert2018correcting].
For the affirmative effects, they use the density of each effect size estimate, given that it is affirmative;
for the non-affirmative effects, they use the density of each effect size estimate, given that it is not affirmative.
...
& \qquad \qquad \qquad - \sum_{i=1}^k \left[(1 - A_i) \log B_i +A_i \log(1 - B_i) \right].
\end{aligned}
$$ {#eq-pUs-objective}
This objective function can be derived directly from the three-parameter selection model. 
...
Conditioning removes all of the information in the likelihood that relates to the selection parameter $\lambda$, so this parameter does not appear in $\mathcal{l}_U(\mu, \tau)$.

One might ask why one should use estimators that condition on $A_1,...,A_k$.


# Equal standard errors

The main simulation results from @VanAert2018correcting are based on a data-generating process where all the effect size estimates have equal standard errors, so $\sigma_1 = \sigma_2 = \cdots = \sigma$.[^The supplementary materials for the paper also include additional simulations that involve varying primary study sample sizes and therefore varying $\sigma_i$.]
...
all the $B_i$'s become constant, with $B_i = B = \Phi\left(\frac{\sigma \Phi^{-1}(1 - \alpha) - \mu}{\sqrt{\tau^2 + \sigma^2}}\right)$ for $i = 1,...,k$.
The maximum likelihood estimator of $\lambda$ (@eq-lambda-score-equation) then simplifies to
$$
\hat\lambda(\mu, \tau) = \frac{k_N (1 - B)}{k_A B}.
$$
The difference between the profile log-likelihood of the 3PSM (@eq-profile-log-likelihood) and the objective function of p-uniform\* (@eq-pUs-objective) also simplifies considerably, to 
$$
\begin{aligned}
\mathcal{l}_P(\mu, \tau) - \mathcal{l}_U(\mu, \tau) &= k_N \log \hat\lambda - k \log\left(1 - B + \hat\lambda B\right) \\
& \qquad \qquad + k_N \log(B) + k_A \log(1 - B) \\
&= k_N \log k_N - k_N \log k_A \\
& \qquad  + k_N \log(1 - B) - k_N \log(B) \\
& \qquad \qquad - k \log k - k \log(1 - B) + k \log(k_A) \\
& \qquad \qquad \qquad + k_N \log(B) + k_A \log(1 - B) \\
...
First, the equivalence of the p-uniform\* and 3PSM estimators  explains why the performance of the point estimators for $\mu$ is so similar (in terms of bias and root mean-squared error) in the main simulations of @VanAert2018correcting.
...
Third, @VanAert2018correcting reported differences between p-uniform\* and 3PSM in the performance of the confidence intervals for $\mu$ in the main simulations, but these must be attributable to differences in how the intervals are constructed 
...
For p-uniform\*, this gives
$$
\begin{aligned}
\frac{\partial \mathcal{u}}{\partial \mu} &=  \sum_{i=1}^k\left(\frac{(T_i - \mu)}{\tau^2 + \sigma_i^2} - \sum_{i=1}^k \frac{\partial B_i}{\partial \mu}\left[\frac{(1 - A_i)}{B_i} - \frac{A_i}{1 - B_i} \right] \\
\frac{\partial \mathcal{u}}{\partial \tau^2} &=  \frac{1}{2} \sum_{i=1}^k\left(\frac{(T_i - \mu)^2 - \left(\tau^2 + \sigma_i^2\right)}{\left(\tau^2 + \sigma_i^2\right)^2}\right) - \sum_{i=1}^k \frac{\partial B_i}{\partial \tau^2}\left[\frac{(1 - A_i)}{B_i} - \frac{A_i}{1 - B_i} \right] 
\end{aligned}
$$

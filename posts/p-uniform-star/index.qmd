...
The authors contrast p-uniform* with the well-known three-parameter selection model (3PSM), which is a special case of the [step-function selection model](/posts/step-function-selection-models/) introduced by @hedges1992modeling.
...
My discussion is necessarily going to get a bit technical.
Consider a meta-analysis that includes $k$ independent effect size estimates, where $t_i$ and $\sigma_i$ denote the observed effect size estimate and corresponding standard error from study $i = 1,...,k$.
...
These assumptions are essentially identical to those of p-uniform*.[^implicit]

[^implicit]: The p-uniform\* method does not explicitly define the $\lambda$, but it is implicit in the assumption that the "weights are the same for the statistically significant and the same for the nonsignificant primary studies’ effect sizes." Although the weights are constant conditional on whether the effects are affirmative, they are allowed to differ between affirmative and non-affirmative effect sizes, and $\lambda$ is the ratio of the weights for non-affirmative effects to the weights for affirmative ones. 
...
$$
d_T(T = t_i, A = a_i | \sigma_i) = \frac{ \lambda^{1 - a_i}}{1 - (1 - \lambda)\beta_i} \times \frac{1}{\sqrt{\tau^2 + \sigma_i^2}}\phi\left(\frac{t - \mu}{\sqrt{\tau^2 + \sigma_i^2}}\right),
$$ {#eq-observed-effect-density}
where $\beta_i = \Phi\left(\frac{\sigma_i \Phi^{-1}(1 - \alpha) - \mu}{\sqrt{\tau^2 + \sigma_i^2}}\right)$.
...
$$
\begin{aligned}
\mathcal{l}(\mu, \tau, \lambda) &= \sum_{i=1}^k \log d_T(T = t_i, A = a_i | \sigma_i) \\
...
The method is an estimator of $\mu$ and $\tau$, but defined by a different objective function that involves truncated normal distributions [Equation 3 of @vanAert2026correcting].
...
Taking the log of their objective function [Equation 4 of @vanAert2026correcting] gives
$$
\begin{aligned}
...
Thus, $\mathcal{u}(\mu, \tau) = \sum_{i=1}^k \log d_T(T = t_i | A = a_i, \sigma_i)$, whereas the 3PSM log-likelihood uses the _unconditional_ density of the observed effect size estimates.
The p-uniform\* method is essentially the maximum likelihood estimator of $\mu$ and $\tau$, conditional on whether each of the effects are affirmative (i.e., conditional on $A_1 = a_1,...,A_k = a_k$.
...
Using conditional distributions for inference is warranted when the conditioning information is an ancillary statistic that carries no information about the model parameters [@Ghosh2010ancillary]. 
However, the statistical significance of the effect size estimates are not ancillary for the average effect size and heterogeneity parameters of the 3PSM. 
@vanAert2026correcting offer no justification for basing inferences about these parameters on the conditional distribution.
It is not obvious to me that confidence intervals based on the conditional distribution would be valid---even asymptotically as $k$ increases (except under conditions where the p-uniform\* objective function coincides with the 3PSM log-likelihood).

# Equal standard errors

The main simulations reported in @vanAert2026correcting are based on a data-generating process where all the effect size estimates have equal standard errors, so $\sigma_1 = \sigma_2 = \cdots = \sigma$.
...
$$
\begin{aligned}
\mathcal{l}_P(\mu, \tau) - \mathcal{u}(\mu, \tau) &= k_N \log \hat\lambda - k \log\left(1 - \beta + \hat\lambda \beta\right) \\
...
First, the equivalence of the p-uniform\* and 3PSM estimators explains why the performance of the point estimators for $\mu$ is so similar (in terms of bias and root mean-squared error) in the main simulations of @vanAert2026correcting
...
Third, @vanAert2026correcting reported differences between p-uniform\* and 3PSM in the performance of the confidence intervals for $\mu$ in the main simulations.
...
How different is the p-uniform\* estimator from the 3PSM maximum likelihood estimator in this case of unequal variances?
The degree of potential discrepancy depends on how different the objective functions are. Allowing for unequal variances
$$
\begin{aligned}
\mathcal{l}_P(\mu, \tau) - \mathcal{u}(\mu, \tau) &= k_N \log \hat\lambda - \sum_{i=1}^k \log\left(1 - (1 - \hat\lambda) \beta_i\right) \\
& \qquad \qquad + \sum_{i=1}^k (1 - A_i) \log(\beta_i) + \sum_{i=1}^k A_i \log(1 - \beta_i) \\
&= \sum_{i=1}^k (1 - A_i) \log \left(\hat\lambda \beta_i\right) + \sum_{i=1}^k A_i \log(1 - \beta_i) \\
& \qquad \qquad  - \sum_{i=1}^k \log\left(1 - (1 - \hat\lambda) \beta_i\right) \\
&= \sum_{i=1}^k (1 - A_i) \log \left(1 - \hat\pi_i\right) + \sum_{i=1}^k A_i \log(\hat\pi_i),
\end{aligned}
$$ {#eq-diff-SE-varying}
where $\hat\pi_i = \frac{1 - \beta_i}{1 - (1 - \hat\lambda)\beta_i}$ is the probability of observing an affirmative result under the three-parameter selection model. The estimated selection parameter is defined by the constraint that $k_A = \sum_{i=1}^k \hat\pi_i$.
Letting $\bar\omega_N = \frac{1}{k_N}\sum_{i=1}^k (1 - A_i)\log(1 - \hat\pi_i)$ and $\bar\omega_A = \frac{1}{k_A}\sum_{i=1}^k A_i\log(\hat\pi_i)$, @eq-diff-SE-varying can be written as
$$
\mathcal{l}_P(\mu, \tau) - \mathcal{l}_U(\mu, \tau) = k_N \bar\omega_N + k_A\bar\omega_A,
$$
with the constraint that $k_A (1 - \bar\pi_A) = k_N \bar\pi_N$, where $\bar\pi_N$ and $\bar\pi_A$ are the average probabilities among the non-affirmative and affirmative effects.
...
However, one could also take this as a deficiency, insofar as the p-uniform\* estimator does not yield information about the strength of selection---a substantively interesting parameter that quantifies the severity of publication bias. 
...
Like the 3PSM MLE, p-uniform\* can be defined by a set of unbiased estimating equations.
Although this property is not sufficient to ensure that p-uniform\* estimator will work well, it does suggest that p-uniform\* might have biases similar to those of the 3PSM MLE.
Furthermore, sandwich variance estimators or non-parametric bootstrapping might be useful for assessing uncertainty in model parameter estimates generated with p-uniform\* [as we examine for MLEs in @pustejovsky2025estimation].
However, it seems unlikely that p-uniform\* would be more efficient than the MLE of the 3PSM under the model's assumptions---maximum likelihood estimation is usually pretty hard to improve upon, at least for low-dimensional problems.
Does p-uniform\* have some robustness advantage that maximum likelihood lacks?
In my view, there needs to be more thorough examination of the properties of p-uniform\* (be it through theoretical analysis or simulation under varied, realistic data-generating processes) before it can be recommended for conducting meta-analyses of real data.

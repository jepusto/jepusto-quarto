...
$$
\begin{aligned}
\mathcal{l}_P(\mu, \tau) - \mathcal{l}_U(\mu, \tau) &= k_N \log \hat\lambda - \sum_{i=1}^k \log\left(1 - (1 - \hat\lambda) \beta_i\right) \\
...
$$
with the constraint that $k_A \bar\pi_A = - k_N \bar\pi_N$, where $\bar\pi_N$ and $\bar\pi_A$ are the average probabilities among the non-affirmative and affirmative effects.

...
I've since been involved with five additional R packages (`lmeInfo`, `metaselection`, `POMADE`, `simhelpers`, and `wildmeta`)---all developed in collaboration with students or colleagues---and my packages related to single-case effect sizes have seen major contributions from student collaborators.
...
* [metaselection](/software/metaselection/) implements several estimators for meta-analytic selection models, with inference methods that handle dependent effect size estimates. This package is brand new so I will exclude it from further analysis.
...
long_from_date <- as.character(as_date(to_date - duration(11, units = "years"))) 
...
long_downloads <- 
  ...
  group_by(package, year, qtr, mth, yr) %>%
  summarize(
    count = sum(count)
  )
...
Here are the monthly download rates for each of these packages (note that the vertical scales of the graphs differ):
...
long_downloads_label <- 
  long_downloads_select %>%
  filter(year == 2026, mth == 9) %>%
  mutate(yr = yr + 0.2)

ggplot(long_downloads_select) + 
  aes(yr, count, color = type, group = package, label = package) + 
  geom_line() + 
  geom_text_repel(
    data = long_downloads_label,
    nudge_x = 0.8
  ) + 
  scale_color_manual(values = c(Pusto = "darkgreen", Comparison = "darkgrey")) +

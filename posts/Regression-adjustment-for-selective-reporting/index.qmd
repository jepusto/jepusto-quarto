...
By my read, the simulation evidence is mixed---they are better than regular random effects models but they are pretty crude adjustments for the bias created by selective reporting, and they don't always work very well.
...
These regression adjustment methods are a bit hard to pin down because they're not full generative models of the process by which the observed set of effect sizes arises.
...
Because of this, I will need to look beyond these specific methods to other models for the selective reporting process if we want to understand how well the regression adjustments work.
...
$$
Substituting into @eq-alpha-expectation, the bias of a regression-adjustment estimator under the step-function selection model is
$$
...
One can apply similar reasoning to get an expression for the variance of the regression adjustment estimator and, combining the bias and the variance, an expression for the root mean squared error (RMSE) of $\hat\alpha$.

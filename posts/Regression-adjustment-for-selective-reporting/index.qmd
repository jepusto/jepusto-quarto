...
[^kinks-with-EK]: With the EK estimator, the kink is an estimated quantity, defined by a preliminary estimator of $\alpha$ and $\hat\tau^2$. This makes is quite tricky to analyze, but I'm going to gloss over that detail entirely (at least for the remainder of this post).
...
I've just thrown down a lot of rather ugly looking formulas, and you will surely be asking yourself why anyone should care about this mess of math.
What are these formulas good for?
To be honest, I worked through this stuff mostly for myself,^[Sometimes, I just have a theoretical itch that you can't help but scratch.]
but I do think there are some useful implications that can be drawn from this.
For one, the expressions for the bias and RMSE show how the performance of the regression adjustments depend on the model parameters, which is useful to know if you're trying to determine how you set up a simulation study involving PET or PEESE, choose which parameters to vary, or fit a meta-model to the results.
...
When conducting a meta-analysis, we will know this sample distribution but won't know the true values of $\mu$, $\tau$, or the parameters of the selection process.
